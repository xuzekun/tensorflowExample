...
def get_test_batch(batch_size,capacity,image_size):
...
def inference(images, batch_size,n_classes):
    ...
    with tf.variable_scope('conv2') as scope:
        weights = tf.get_variable("weights", shape=[3,3,16,32], dtype=tf.float32,initializer=tf.truncated_normal_initializer(stddev=0.1))
        biases = tf.get_variable("biases", shape=[32], dtype=tf.float32,initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.variable_scope('conv3') as scope:
        weights = tf.get_variable("weights", shape=[3,3,32,64], dtype=tf.float32,initializer=tf.truncated_normal_initializer(stddev=0.1))
        biases = tf.get_variable("biases", shape=[64], dtype=tf.float32,initializer=tf.truncated_normal_initializer(stddev=0.1))
    ...
    with tf.variable_scope('local5') as scope:
        weights = tf.get_variable('weights',shape=[256, n_classes], dtype=tf.float32, initializer=tf.truncated_normal_initializer(stddev=0.005, dtype=tf.float32))
...
def test():
    TEST_BATCH_SIZE = 100
    test_img_batch = get_test_batch(TEST_BATCH_SIZE, CAPACITY,IMAGE_SIZE)
    ...
    with tf.Session() as sess:
        ...
        saveDataFrame = pd.DataFrame({'id': range(1, result.shape[0] + 1), 'label': result},
                                     columns=['id', 'label'])
        saveDataFrame.to_csv('0710.csv', index=False)
