...
tl.files.load_and_assign_npz(sess=sess, name='./modecd.npz', network=net)
...
test_img = tf.train.batch([img], batch_size=10, capacity=200)
...
index = 0
for i in range(1250):
    imgs = sess.run(test_img)
    out = sess.run(probs,feed_dict={x:imgs})
    for j in out:
        result[index] = j[1]
        index += 1
...
saveDataFrame = pd.DataFrame({'id': range(1, result.shape[0] + 1), 'label': result},
                                     columns=['id', 'label'])
saveDataFrame.to_csv('0802.csv', index=False)
